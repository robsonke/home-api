...
export class AirfoilRouter extends RestRouter {
  ...
  initRoutes() {
    ...
    this.router.put('/speakers/:id/connect', (request: Request, response: Response, next: any) => this.airfoilCtrl.enableSpeaker(request, response));
    this.router.put('/speakers/:id/disconnect', (request: Request, response: Response, next: any) => this.airfoilCtrl.disableSpeaker(request, response));
